  ...
  drawRect=TRUE, las=1)
{
    ...
    for(i in 1:n) {
        ...
        #
        hscale <- 0.4/max(smout$estimate) * wex[i %% (length(wex) + 1)]
    ...
    if(!horizontal) {
      if(!add){
        ...
        axis(2, lwd=lwd)
        axis(1,at = at, label=label ,las=las, lwd=lwd)
      }
      ...
      for(i in 1:n) {
          ...
          if(drawRect){
            ...
            # plot 50% KI box
            rect( at[i]-boxwidth[i]/2, q1[i], at[i]+boxwidth[i]/2, q3[i], col=rectCol)
      ...
      if(!add){
        ...
        axis(1, lwd=lwd)
        axis(2,at = at, label=label ,las=las, lwd=lwd)
      }
